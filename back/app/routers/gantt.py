...
async def get_gantt(doc_id: uuid.UUID, db=Depends(get_db), user=Depends(current_user)):
...
async def create_task(doc_id: uuid.UUID, task: GanttTaskCreate, db=Depends(get_db), user=Depends(current_user)):
...
async def update_task(doc_id: uuid.UUID, task_id: uuid.UUID, task: GanttTaskUpdate, db=Depends(get_db), user=Depends(current_user)):
    fields = []
    values = []
    idx = 1
...
async def delete_task(doc_id: uuid.UUID, task_id: uuid.UUID, db=Depends(get_db), user=Depends(current_user)):
...
async def bulk_update_tasks(doc_id: uuid.UUID, tasks: List[dict], db=Depends(get_db), user=Depends(current_user)):
    for t in tasks:
        task_id = t.get("id")
...
async def get_resources(doc_id: uuid.UUID, db=Depends(get_db), user=Depends(current_user)):
...
async def create_resource(doc_id: uuid.UUID, res: GanttResourceCreate, db=Depends(get_db), user=Depends(current_user)):
    record = await db.fetchrow("""
